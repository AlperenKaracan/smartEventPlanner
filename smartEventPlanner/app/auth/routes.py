...
from app.email_service import send_confirmation_email, send_reset_email
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        point = Point(user_id=user.id, score=10, badge='Yeni Üye')
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.password.data):
            if user.is_confirmed:
                ...
                flash('Lütfen önce e-postanızı doğrulayın. Doğrulama e-postası yeniden gönderildi.', 'warning')
                token = generate_confirmation_token(user.email)
                send_confirmation_email(user, token)
                return redirect(url_for('auth.login'))
        else:
...
def profile():
    ...
    return render_template('profile.html', form=form, profile_image=profile_image, points=points)
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            flash('Bu e-posta ile kayıtlı bir kullanıcı bulunamadı.', 'warning')
...
def view_profile(user_id):
    ...
    points = user.points
    return render_template('profile_view.html', user=user, profile_image=profile_image, points=points)
